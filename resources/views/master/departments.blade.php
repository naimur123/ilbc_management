...
<div class="d-flex justify-content-between mb-3">
    <h4>Departments</h4>
    <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#addModal"><i class="bi bi-plus-lg"></i> Add Department</button>
</div>
<div class="kpi-card">
    <table class="table table-sm align-middle">
        <thead><tr><th>Name</th><th>Code</th><th>Status</th><th class="text-end">Action</th></tr></thead>
        <tbody>
        ...
        <tr>
            ...
            <td class="text-end">
                ...
                <form action="{{ route('departments.destroy', $item) }}" method="POST" class="d-inline" onsubmit="return confirm('Delete this department?')">@csrf @method('DELETE')<button class="btn btn-sm btn-outline-danger"><i class="bi bi-trash"></i></button></form>
            </td>
        </tr>
        @endforeach
        </tbody>
...
<div class="modal fade" id="editModal{{ $item->id }}"><div class="modal-dialog"><div class="modal-content">
    <form action="{{ route('departments.update', $item) }}" method="POST">@csrf @method('PUT')
