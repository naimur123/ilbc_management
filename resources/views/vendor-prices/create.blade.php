...
<form action="{{ route('vendor-prices.store') }}" method="POST" class="kpi-card">
    ...
    <div class="row g-3">
        ...
        <div class="col-md-4"><label class="form-label">Product SKU *</label>
            <select class="form-select" name="product_sku_id" required><option value="">-</option>
            @foreach($skus as $s)<option value="{{ $s->id }}">{{ $s->product->name }} ({{ $s->sku_code }})</option>@endforeach
            </select>
        </div>
    ...
    <div class="mt-3"><button class="btn btn-primary">Save Price</button> <a href="{{ route('vendor-prices.index') }}" class="btn btn-light">Cancel</a></div>
</form>
@endsection
